    ...
    <div class="login-box">
      ...
      <form>
        <div class="user-box">
          ...
          <label style="border-bottom: 80px">账号</label>
        </div>
        <div class="user-box">
...
export default {
  ...
  methods: {
    ...
    handleSubmit() {
     //登录按钮
      axios.post('http://'+store.baserUrl+'/hrms_verify/companymanager/login',{'account':this.account,'password':this.password})
          .then(response=>{
            if(response.data.data.msg=='登录成功') {
              jsCookie.set('token', response.data.data.token, {domain: 'localhost'})
